...
public:
    ...
    // Constructor that initializes the player's name and starting location

    string getName() const;
    // Getter method to retrieve the name of the player

    string getCurrentLocation() const;
    // Getter method to retrieve the current location of the player

    LinkedList& getInventory();
    // Getter method to retrieve a reference to the player's inventory

    void move(const string& newLocation);
    // Method to update the player's current location to the specified new location
};
